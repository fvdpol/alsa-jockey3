...
> **Superseded 2026-08-29 by `re/on-demand_streaming.md`, then closed the same
> day.** On-demand streaming was built with a restart treated as a full cold
> start rather than the 20 ms budget below, so the escalation ladder, the
> budget and the stop rules here never applied, and the temporary dev-only
> hook was never built (a runtime-writable `idle_timeout` on the real feature
> answered the same question without code that never ships). What survived,
> promoted from a secondary check to *the* gate -- whether MIDI IN keeps
> arriving on EP 0x83 while the PCM URBs are stopped -- was tested on hardware
> and came back no. The feature is closed; see `re/on-demand_streaming.md`.
