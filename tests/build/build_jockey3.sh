...
# ------------------------------------------------------------- build manifest
# Maps the module's srcversion to the revision that produced it. srcversion is
# a hash over the module source and is readable from a loaded module via
# /sys/module/.../srcversion, so this is what lets a test run on another
# machine say exactly which commit it tested -- without adding anything to the
# driver for the benefit of tests.
#
# Keyed by GNU build-id, not by srcversion. srcversion is only emitted when
...
# and the kernel exposes module notes under /sys/module/<m>/notes/, so a test
# run can read it back from the loaded module on any of them.
read_build_id() {
	local ko=$1 v=""
	...
	if [ -z "$v" ] && command -v python3 >/dev/null 2>&1; then
		v=$(python3 - "$ko" <<-'PY'
		import struct, sys
		# Locate .note.gnu.build-id via the section headers.
...
KO=$DST/snd-reloop-jockey3.ko
if [ -f "$KO" ]; then
	srcver=$(read_build_id "$KO")
	if [ -n "$srcver" ]; then
		mkdir -p "$MANIFEST_DIR"
		# Both worktree and index: `git diff` alone reports clean when
		# changes are merely staged, which would label a modified build
		# as pristine.
		dirty=false
		[ -z "$(git -C "$SRC_DIR" status --porcelain 2>/dev/null)" ] || dirty=true
		cat > "$MANIFEST_DIR/$srcver.json" <<EOF
{
  "build_id": "$srcver",
  ...
  "kernel_release": "$(make -s kernelrelease 2>/dev/null)",
  "arch": "$(uname -m)"
}
EOF
		echo "manifest: $MANIFEST_DIR/$srcver.json"
	else
		echo "  (could not read a build-id from $KO -- test runs will not"
		echo "   be able to resolve the loaded module to a git revision)"
	fi
fi
