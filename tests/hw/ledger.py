...
INDEX_CACHE_NAME = ".ledger_index_cache.json"


def _index_entry(data):
    """The slice of a run.json that build_index() actually looks at.
...
def load_runs_indexed(root):
    """Like load_runs(), but only as much of each run as build_index() needs.

    `--matrix` only ever asks for the latest pass/attempt per (target, case),
    yet used to re-read and re-parse every run.json under results/ -- runs it
    already knew about as much as brand new ones -- which made it slower with
    every test ever recorded instead of with every target/case pair. This
    caches the small extract above, keyed by path plus mtime+size, so a run
    directory that has not changed since the last invocation is never opened
    again. A run.json *can* change after the fact (checklist.py fills in a
    manual verdict in place), which is why the check is a stamp comparison
    rather than "seen this path before" -- mtimes are unreliable across an
    rsync'd copy of results/, but that only costs an extra re-read, never a
    stale answer.
    """
    cache_path = os.path.join(root, INDEX_CACHE_NAME)
    try:
    ...
    for path in sorted(glob.glob(os.path.join(root, "*", "*", "run.json"))):
        ...
        try:
            ...
            changed = True
            continue
        entry = _index_entry(data)
        entry["_stamp"] = stamp
        entries[path] = entry
        changed = True

    stale = set(entries) - seen
    for path in stale:
        del entries[path]
    ...
    return [dict(entries[path], _path=path) for path in sorted(entries)]
...
def main():
    ...
    if args.matrix:
        ...
        # the bench a moment ago. Only the latest pass/attempt per
        # (target, case) is needed, so use the cached lightweight extract
        # instead of re-parsing every run.json ever recorded.
        entries = load_runs_indexed(root)
        if not entries:
            print(f"no runs found under {root}")
            print("run ./runner.py --profile smoke to create one")
            return 0
        clean_idx = build_index(entries, cases, clean_only=True)
