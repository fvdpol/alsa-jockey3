...
def run(c, direction, device):
    ...
    # whenever a case's "xruns"/"xruns_playback"/"xruns_capture" metric is
    # nonzero, to flag an incidental hiccup worth a longitudinal look (see
    # lib/results.py). Here the xrun count is the thing under test, not a
    # side effect, so that rebadge would fire on every successful run.
    c.metric("xruns_injected", watch.xruns)
    c.metric("avail_max", watch.avail_max)

    # An xrun beyond the ones this case itself injected is not this case's
    # functional criterion to fail on -- it is exactly the "ran fine, but
    # had a hiccup worth a second look" case PASS_XRUN exists for, so it
    # goes through the normal metric-driven rebadge instead of c.fail().
    unexpected = max(0, watch.xruns - injections)
    c.metric(f"xruns_{direction}", unexpected)
